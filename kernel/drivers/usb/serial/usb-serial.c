			       ...
			       const struct usb_device_id *id)
{
    ...
    /* Add for Longsung U8300 LTE modem, add by Aaron */
    if(dev->descriptor.idVendor == LONGSUNG_VENDOR_ID &&
        dev->descriptor.idProduct == LONGSUNG_U8300_PRODUCT_ID &&
        interface->cur_altsetting->desc.bInterfaceNumber == 4) {
        dev_err(ddev, "Discover the 4th interface for U8300 NDIS\n");
        return -EDOM;
    }
