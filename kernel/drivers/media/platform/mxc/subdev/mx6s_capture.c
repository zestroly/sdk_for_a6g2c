...
static void csi_init_interface(struct mx6s_csi_dev *csi_dev)
{
    ...
    val = 0;
    __raw_writel(val, csi_dev->regbase + CSI_CSICR2);
    ...
    val = 0;
    val |= 0x00 << 18;
    __raw_writel(val, csi_dev->regbase + CSI_CSICR18);
}
...
#ifdef CONFIG_ADV7180
    cr2 |= 0x40000000;  /* set to INCR4 for: 720(pix) x2 = 1440 bytes = 360words, 360 / 4 = 90 */
#else
	cr2 |= 0xC0000000;
#endif
...
#ifdef CONFIG_ADV7180
    u32 t_width = 0;
    t_width = (csi_dev->pix.width/4) << 16; // Aaron test
#endif

	if (pix->field == V4L2_FIELD_INTERLACED) {
		csi_deinterlace_enable(csi_dev, true);
#ifdef CONFIG_ADV7180
		csi_buf_stride_set(csi_dev, t_width);
#else
